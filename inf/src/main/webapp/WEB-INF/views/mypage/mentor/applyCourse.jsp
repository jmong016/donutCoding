	...
	<section id="mypage-content">
		<div id="mento-course">
			<div class="tab">
				<div>
					<button class="btn btn-outline-primary tablinks"
						onclick="openTab(event, 'c-able')">판매중인 강의</button>
					<button class="btn btn-outline-primary tablinks"
					...
					<c:choose>
						<c:when test="${ableCourse != null}">
							<c:forEach var="i" items="${ableCourse }">
								<div class="flex-item" onclick="location.href='${path}/course/${i.course_seq }'" style="cursor: pointer;">
				...
				<div class="apply-wrapper">
					...
					<div class="price-app">
						...
						<div class="flex-wrap">
							...
							<div class="price method-check method-free-check">
								<input name="course_price" id="course_price_free" type="number"
									size="10" placeholder="무료" disabled="disabled"/>
							</div>
							<div class="price method-check method-common-check">
								<input name="course_price_pay" id="course_price_pay" type="number"
					...
					<div class="thumb-app">
						...
						<div class="flex-wrap">
							...
							<div id="thumb_selected"><img id="preview" style="width: 50%;object-fit:cover;border-radius: 1em;"></div>
						</div>
					</div>
					<!-- 강의명 -->
					<div class="nm-app">
			...
			<div id="c-cancle" class="tabcontent">
				<h5 class="p-3" style="font-weight: bold;">강의 (신청 취소)</h5>
						...
						<c:when test="${cancleCourse != null}">
							<c:forEach var="i" items="${cancleCourse }">
								<div class="flex-item">
									...
									<div class="reapply_btn_container" style="padding-left: 1em;margin: auto 0;">
												<button type="button" class="btn btn-info white-font" onclick="reapplyCourse(${i.course_seq})">강의 재신청</button>
									</div>
								</div>
							</c:forEach>
						</c:when>
...
<script type="text/javascript">
	...
	// 정가 , 할인가 숫자 검증
	$("#course_price_pay").on("input",function() {
	...
	$("#course_sales_price").on("focus", function() {
		var price = $("#course_price_pay").val();
		if (price == null || price == "") {
	...
	$("#course_sales_price").on("change", function() {
		var price = parseInt($("#course_price_pay").val());
	...
	$("#sale-cal").on("click", function() {
		...
		var p = parseInt($("#course_price_pay").val());
	...
	$("#thumb_select").on("change",function(){
		...
		var regex = new RegExp("(.*?)\.(png|PNG|jpg|JPG|jpeg|JPEG|svg|SVG|gif|GIF)$");
	...
	$("#c-submit").on("click", function() {
		...
		/* 정가 판매 */
			...
			if(isNull("course_price_pay")){
				alert("정가를 입력하세요.");
				$("#course_price_pay").focus();
				return false;
			}else{
				price = parseInt($("#course_price_pay").val());
				alert(price);
				sales_price = 0;
				rating = 0;
			}
	...
	function reapplyCourse(seq){
		$.ajax({
			url : "${path}/mypage/reapplyCourse",
			type : "POST",
			dataType : "json",
			data : {"course_seq" : seq },
			success : function(data) {
